document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadSpeciesData() {
    try {
      ...
      const region = [...new Set(allSpecies.flatMap((e) => e.region))];
      ...
      const category = [...new Set(allSpecies.flatMap((e) => e.category))];
      ...
      const status = [...new Set(allSpecies.flatMap((e) => e.status))];
    ...
    speciesToShow.forEach((species) => {
      ...
      card.innerHTML = `
                ...
                <div class="species-card-content">
                    <h3>${species.name}</h3>
                    <span class="species-status">${getStatusLabel(
                      species.status
                    )}</span>
  ...
  function showSpeciesDetails(speciesId) {
    ...
    document.getElementById("modalSpeciesStatus").textContent = getStatusLabel(
      species.status
    );
    document.getElementById("modalSpeciesDesc").textContent =
      species.description;
    ...
    species.images.forEach((img, index) => {
      ...
      thumbnail.addEventListener("click", () => {
        ...
        document
          .querySelectorAll("#speciesThumbnails img")
